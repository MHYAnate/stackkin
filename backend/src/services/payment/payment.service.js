...
import logger from '../../config/logger.js';
import { PaymentError, ValidationError, NotFoundError } from '../../errors/index.js';
...
class PaymentService {
  ...
  async createPayment(paymentData) {
    try {
      return await this.orchestrator.processPayment({
        ...paymentData,
        gateway: 'zainpay', // Default to Zainpay
  ...
  async verifyPayment(reference, userId) {
    try {
      return await this.orchestrator.verifyPayment(reference, userId);
    } catch (error) {
      logger.error('Verify payment error:', error);
      throw error;
    }
  ...
  async refundPayment(refundData) {
    try {
      const { paymentId, amount, reason, metadata = {} } = refundData;
      ...
      // Get gateway
      const gateway = PaymentGatewayFactory.create(originalPayment.gateway);
      
      // Process refund
      const refundResponse = await gateway.refundPayment({
        paymentId: originalPayment.gatewayRef,
        amount,
        reason,
        metadata: {
          ...metadata,
      ...
      const refundTransaction = await Transaction.create({
        ...
        txnRef: `REFUND_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
        type: 'refund',
        amount: -amount, // Negative amount for refund
        currency: originalPayment.currency,
        status: refundResponse.success ? 'processing' : 'failed',
      ...
      originalPayment.metadata.refunds = [
        ...(originalPayment.metadata.refunds || []),
        {
      ...
      return {
        ...
        gatewayReference: refundResponse.reference,
        timestamp: new Date().toISOString()
      };
  ...
  async getPaymentStatistics(startDate, endDate, groupBy = 'DAY') {
    try {
      ...
      const [paymentStats, transactionStats, gatewayStats] = await Promise.all([
        // Payment statistics
        Payment.aggregate([
          { $match: dateFilter },
          {
      ...
      const successfulPayments = paymentStats
        .filter(stat => stat._id === 'success')
        .reduce((sum, stat) => sum + stat.count, 0);
      
      const totalVolume = paymentStats.reduce((sum, stat) => sum + stat.totalAmount, 0);
      ...
      return {
        ...
        totalVolume,
        averageTransactionValue: totalPayments > 0 ? totalVolume / totalPayments : 0,
        conversionRate: totalPayments > 0 ? (successfulPayments / totalPayments) * 100 : 0,
        
        byStatus: paymentStats.reduce((acc, stat) => {
          acc[stat._id] = {
            count: stat.count,
            volume: stat.totalAmount,
            percentage: totalPayments > 0 ? (stat.count / totalPayments) * 100 : 0
          };
          return acc;
        }, {}),
        ...
        byGateway: gatewayStats.reduce((acc, stat) => {
          acc[stat._id] = {
            count: stat.count,
            volume: stat.totalAmount,
        ...
        timeframe: { startDate, endDate, groupBy },
  ...
  async transferFunds(transferData) {
    try {
      const { fromUserId, toUserId, amount, narration } = transferData;
      ...
      const senderTransaction = await Transaction.create({
        userId: fromUserId,
        txnRef: `TRANSFER_OUT_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
        type: 'transfer',
        amount: -amount,
        currency: 'NGN',
        status: 'success',
        ...
        description: `Transfer to user ${toUserId}: ${narration}`,
        metadata: {
          toUserId,
          narration,
          receiverTransactionId: null // Will update after creation
        }
      });
      
      const receiverTransaction = await Transaction.create({
        userId: toUserId,
        txnRef: `TRANSFER_IN_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
        type: 'transfer',
        amount,
        currency: 'NGN',
        status: 'success',
        gateway: 'internal',
        paymentMethod: 'wallet',
        description: `Transfer from user ${fromUserId}: ${narration}`,
        metadata: {
          fromUserId,
          narration,
      ...
      senderTransaction.metadata.receiverTransactionId = receiverTransaction._id;
  ...
  async handleZainpayWebhook(payload, signature) {
    try {
      const gateway = PaymentGatewayFactory.create('zainpay');
      
      // Validate webhook signature
      const isValid = await gateway.validateWebhook(signature, payload);
      if (!isValid) {
      ...
      logger.info('Processing Zainpay webhook:', { event, txnRef: data.txnRef });
      ...
      switch (event) {
        ...
        case 'card.payment.success':
          await this.handleCardPaymentSuccess(data);
          break;
      ...
      return { processed: true, event, timestamp: new Date().toISOString() };
  ...
  async handleDepositSuccess(data) {
    ...
    if (virtualAccount) {
      ...
      virtualAccount.depositedAmount = data.depositedAmount ? 
        parseFloat(data.depositedAmount) / 100 : 0;
      virtualAccount.senderName = data.senderName;
      ...
      if (transaction) {
        transaction.status = 'success';
        transaction.completedAt = new Date();
        transaction.gatewayRef = data.paymentRef;
  ...
  async handleCardPaymentSuccess(data) {
    const Transaction = (await import('../../models/Transaction.js')).default;
    ...
    if (transaction) {
      ...
      if (data.cardToken) {
        transaction.metadata.cardToken = data.cardToken;
